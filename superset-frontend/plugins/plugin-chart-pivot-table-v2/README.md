...
- **Column totals formatting:** `Data -> Options` no longer keeps subtotal-formatting controls and no longer shows `Show row subtotals`. For `Columns total`, the UI uses `Per-Metric Total Overrides` plus common `Total label`, `Total font color`, and `Total background color`.
- **Column totals formatting:** `Data -> Options` no longer keeps subtotal-formatting controls and no longer shows `Show row subtotals`. For `Columns total`, the UI uses `Per-Metric Total Overrides` plus common `Total label`, `Total font size (px)`, `Total font color`, and `Total background color`. `Show columns subtotal` now exposes `Columns subtotal label`, `font size`, `font color`, and `background color`.
...
Version in `package.json` is bumped when shipping plugin changes in this fork; the chart metadata string in `src/plugin/index.ts` should stay in sync for support/debugging. Current plugin version after the formatting controls update: `0.0.106`.
