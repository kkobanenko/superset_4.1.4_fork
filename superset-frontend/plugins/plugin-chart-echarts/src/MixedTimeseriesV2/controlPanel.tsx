...
function createCustomizeSection(
  ...
  return [
    ...
    [
      {
        ...
        config: {
          ...
          visibility: ({ controls }: ControlPanelsContainerProps) =>
            ...
            Boolean(controls?.[`stack${controlSuffix}`]?.value),
        },
      },
    ],
    [
      {
        name: `stackedLabelPosition${controlSuffix}`,
        config: {
          type: 'SelectControl',
          label: t('Stacked Label Position'),
          renderTrigger: true,
          default: 'top',
          choices: [
            ['top', t('Top')],
          ...
          description: t(
            'Position of labels in stacked bar chart: Top (above column) or Middle (inside each stack segment)',
          ),
          visibility: ({ controls }: ControlPanelsContainerProps) =>
            Boolean(controls?.[`stack${controlSuffix}`]?.value) &&
    ...
    [
      {
        name: `hideZeroValues${controlSuffix}`,
        config: {
          type: 'CheckboxControl',
          ...
          default: false,
          description: t(
            'Hide labels for zero values regardless of label position',
          ),
          visibility: ({ controls }: ControlPanelsContainerProps) =>
            Boolean(controls?.[`stack${controlSuffix}`]?.value) &&
            controls?.[`seriesType${controlSuffix}`]?.value ===
    ...
    [
      {
        name: `labelFontSize${controlSuffix}`,
        config: {
          type: 'SelectControl',
          ...
          description: t(
            'Font size for value labels: Normal (12px), Small (8px), Micro (5px)',
          ),
          visibility: ({ controls }: ControlPanelsContainerProps) =>
            Boolean(controls?.[`show_value${controlSuffix}`]?.value),
        },
