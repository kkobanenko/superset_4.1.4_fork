...
## Проблема 10: Использование критериев из PivotData (подсказки №1 и №2)

### Подсказка №1
Колонки для скрытия - это **подмножество колонок**, которые включаются/выключаются параметром Data -> Options -> Show columns subtotal. Нужно использовать тот же подход, что и PivotData.

### Подсказка №2
Между колонками со значениями "Дата" могут находиться колонки для других полей и их подытогов. Нужно исследовать **ВСЕ** колонки таблицы.

### Решение
Использованы критерии из PivotData (utilities.js, isColSubtotalOrTotal):

1. **Subtotal колонка**: `colKey.length < colAttrs.length` (как в PivotData)
2. **Subtotal для colAttrs[attrIdx]**:
   - Критерий A: `colKey.length === attrIdx + 1` (PivotData: подытог для группы colAttrs[attrIdx])
   - Критерий B: `colKey[attrIdx + 1] === 0` (маркер 0: агрегируем по colAttrs[attrIdx+1], подытог для colAttrs[attrIdx])

Фильтрация применяется ко **всем** visibleColKeys (подсказка №2). Логика в valueCells.map() приведена к тем же критериям.

### Файлы изменений
- `TableRenderers.jsx`: метод `render()` - новая функция `isSubtotalForAttr()`, упрощенная фильтрация
- `TableRenderers.jsx`: `valueCells.map()` - функция `isSubtotalForAttrCell()` с теми же критериями
...
- ✅ **Завершено**: Реализованы критерии из PivotData (подсказки №1, №2) - colKey.length и colKey[attrIdx+1]===0
- ⏳ **Ожидается**: Тестирование фичи в браузере после пересборки Docker

---

**Дата создания документа**: 2026-02-05  
**Последнее обновление**: 2026-02-06 (Проблема 10: критерии из PivotData, подсказки №1 и №2)  
